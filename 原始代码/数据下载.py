...
class CopernicusAPI:
    ...
    def search(self, wkt, date_from, date_to, product_type, max_results=100,
               platforms=None, orbit_dirs=None, polarisations=None):
        filters = [
        ...
        if platforms:
            plat_filter = " or ".join(
                f"startswith(Name,'{p}')" for p in platforms
            )
            filters.append(f"({plat_filter})")

        url = (f"{SEARCH_URL}?$filter={requests.utils.quote(' and '.join(filters))}"
        ...
        # 客户端过滤轨道方向（OData不直接支持）
        # Sentinel-1 产品名中第 35 位字符：A=升轨(Ascending) D=降轨(Descending)
        # 例: S1A_IW_GRDH_1SDV_20250721T..._**A**_...
        result = []
        for p in products:
            if orbit_dirs:
                name = p.get("Name", "")
                # 从 Attributes 列表中读取 orbitDirection（最准确）
                orbit_val = ""
                for attr in p.get("Attributes", []):
                    if attr.get("Name") == "orbitDirection":
                        orbit_val = attr.get("Value", "").upper()
                        break
                # 若 Attributes 中没有，退回到产品名第35位字符粗略判断
                if not orbit_val and len(name) > 35:
                    orbit_val = "ASCENDING" if name[35].upper() == "A" else "DESCENDING"
                # orbit_dirs 传入示例: ["ASC"] 或 ["DES"] 或 ["ASC","DES"]
                want_asc = any(d.upper().startswith("ASC") for d in orbit_dirs)
                want_des = any(d.upper().startswith("DES") for d in orbit_dirs)
                is_asc   = "ASCENDING" in orbit_val
                is_des   = "DESCENDING" in orbit_val
                if not ((want_asc and is_asc) or (want_des and is_des)):
                    continue   # 不符合轨道方向要求，跳过
            result.append(p)
        return result
...
class App(tk.Tk):
    ...
    def _build_search_tab(self):
        ...
        ttk.Checkbutton(pf2, text="S1A", variable=self.var_s1a).pack(side="left", padx=(0,10))
        ttk.Checkbutton(pf2, text="S1B", variable=self.var_s1b).pack(side="left")
        ...
        # 结果 Treeview
        cols = ("sel","name","date","platform","pol","size","online")
        ...
        self.tree.column("date",     width=155, anchor="center")
        self.tree.column("platform", width=55,  anchor="center")
        self.tree.column("pol",      width=70,  anchor="center")
        self.tree.column("size",     width=70,  anchor="center")
        self.tree.column("online",   width=60,  anchor="center")
    ...
    def _do_search(self):
        ...
        def _run():
            try:
                results = api.search(wkt, date_from, date_to, product_type,
                                     max_results, platforms)
    ...
    def _render_results(self):
        ...
        for i, p in enumerate(self.search_results):
            ...
            plat   = "S1A" if name.startswith("S1A") else "S1B"
            pol    = "VV+VH"
            size   = f"{p.get('ContentLength', 1700*1024*1024)/1024**3:.1f}GB"
            online = "✓ 在线" if p.get("Online", True) else "归档"
            tag    = "even" if i % 2 == 0 else "odd"
            inq    = " [已加入]" if p["Id"] in queue_ids else ""
            iid = self.tree.insert("", "end", iid=str(i),
                                   values=("", name, date+" UTC", plat, pol, size, online+inq),
                                   tags=(tag,))

        self.lbl_count.config(text=f"搜索结果：{len(self.search_results)} 景")
        self.set_status(f"搜索完成，共 {len(self.search_results)} 景")
    ...
    def _start_download(self):
        ...
        def _run():
            ...
            # 用一个共享 api 实例会有 token 竞争，每个 worker 独立一个
            worker_apis = [CopernicusAPI() for _ in range(n_workers)]
            for wa in worker_apis:
                wa.token      = api.token
                wa.token_time = api.token_time
            ...
            def _download_one(args):
                ...
                wapi = worker_apis[slot_idx % n_workers]
                name = q["name"]
            ...
            self.after(0, lambda: self.lbl_prog.config(text=f"完成！成功 {ok_cnt}/{total} 景"))
            self.after(0, lambda: self.btn_start.config(state="normal"))
            self.after(0, lambda: self.btn_stop.config(state="disabled"))
            self.after(0, lambda: self.cmb_parallel.config(state="readonly"))
            self.downloading = False
