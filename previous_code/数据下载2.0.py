...
class App(tk.Tk):
    ...
    def _render_results(self):
        ...
        for i, p in enumerate(self.search_results):
            ...
            plat   = "S1A" if name.startswith("S1A") else ("S1C" if name.startswith("S1C") else "S1B")
            # 从 Attributes 读取极化（避免硬编码）
            pol = "—"
            for attr in p.get("Attributes", []):
                if attr.get("Name") == "polarisationChannels":
                    pol = attr.get("Value", "—")
                    break
            if pol == "—":   # 退回产品名推断：_1SDV_ → VV+VH，_1SSV_ → VV
                pol = "VV+VH" if "DV" in name else ("HH+HV" if "DH" in name else "单极化")
